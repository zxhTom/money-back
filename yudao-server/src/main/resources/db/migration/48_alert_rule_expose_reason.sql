...
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `custom_alert_rule` WHERE `alert_type` = 'IP_BLACKLIST_BLOCK' AND `deleted` = 0);
...
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM `custom_alert_rule` WHERE `alert_type` = 'CONTRACT_QUERY_RATE_LIMIT' AND `deleted` = 0);
